...
# ⚠️ THIS PROJECT KEEPS ITS DESIGN DOCS IN `docs/`, NOT `design/gdd/`.
# It predates the template's layout and has ~25 of them (ARENA_DESIGN, ABILITY_REWORK,
# BALANCING, SPATIAL_COMBAT_DESIGN, GODOT_MIGRATION...). Counting only `design/gdd/` reported
# "0 design docs" against a heavily-documented codebase every session — a false alarm that
# trains you to ignore the real ones. Count both.
if [ -d "design/gdd" ] || [ -d "docs" ]; then
  DESIGN_FILES=$(( $(find design/gdd -type f -name "*.md" 2>/dev/null | wc -l) + $(find docs -maxdepth 1 -type f -name "*.md" 2>/dev/null | wc -l) ))
else
  DESIGN_FILES=0
fi
