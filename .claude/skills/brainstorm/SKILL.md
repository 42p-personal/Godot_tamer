...
- **Target platform**: Use `AskUserQuestion` — "What platforms are you targeting for this game?"
  ...
  Note platform implications if relevant. ⚠️ THIS STUDIO IS GODOT-ONLY — the engine question is already answered; do not reopen it.

- **Engine experience**: Use `AskUserQuestion` — "Do you already have an engine you work in?"
  Options: `Godot` (the studio's engine — there is no second option)
