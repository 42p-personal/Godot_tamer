...
⚠️ THIS PROJECT'S GODOT TESTS RUN HEADLESS: `cd monster-tamer && ./run_contract.sh`. Its
...
PASS/FAIL counts. If no artifacts exist: "the contract runner must be run from the
editor or CI pipeline. Please confirm test status manually before proceeding."

```bash
# List most recent contract-run logs (bash) — on Windows PowerShell use the fallback below
