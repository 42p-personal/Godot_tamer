...
const POOLS: Record<Stat, Row[]> = {
  ...
  DEX: [
    ...
    { name: 'Shadowstep', learnLevel: 120, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 5.2, accuracy: 92, power: 42, mana: 16, variance: 0.15, range: 2.8, projectile: { speed: 130, width: 0.2, pierce: 0 }, desc: 'Steps through the shadow behind them. The only reliable way past a front line.' },
    { name: 'Ambush', learnLevel: 200, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 3.9, accuracy: 92, power: 39, mana: 18, variance: 0.2, effects: { firstStrikeMult: 1.5 }, range: 2.8, projectile: { speed: 120, width: 0.21, pierce: 0 }, desc: 'Devastating on someone who has not swung yet — worthless once they have seen you.' },
    ...
    { name: 'Smoke Bomb', learnLevel: 340, type: 'debuff', channel: 'ranged', target: 'allEnemies', cooldown: 6.5, accuracy: 100, power: 0, mana: 20, effects: { accDebuff: 14, duration: 2 }, status: { kind: 'blind', chance: 50, duration: 2 }, range: 2.4, projectile: { speed: 85, width: 0.16, pierce: 0 }, desc: 'Blinds everything close and covers the exit — the setup Ambush wants.' },
    { name: 'Hamstring', learnLevel: 420, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 3.9, accuracy: 90, power: 47, mana: 16, variance: 0.15, range: 2.7, projectile: { speed: 130, width: 0.21, pierce: 0 }, desc: 'Cuts the leg out from under them. They keep fighting; they stop leaving.' },
    { name: 'Throat Cut', learnLevel: 600, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 6.5, accuracy: 90, power: 74, mana: 30, variance: 0.1, status: { kind: 'silence', chance: 55, duration: 2 }, range: 2.5, projectile: { speed: 140, width: 0.22, pierce: 0 }, desc: 'Quiet, precise, and it ends the casting. What the Assassin line exists to do.' },
    { name: 'Heartseeker', learnLevel: 850, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 5.2, accuracy: 92, power: 36, mana: 26, variance: 0.3, effects: { hits: [2, 3], execute: 0.35 }, range: 2.8, projectile: { speed: 109, width: 0.2, pierce: 0 }, desc: 'Two or three finding strikes, lethal against anything already failing.' },

    // ── Venomcraft — put poison on, keep it there, then detonate it ───────────
    { name: 'Piercing Shot', learnLevel: 90, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 2.6, accuracy: 90, power: 17, mana: 14, variance: 0.2, effects: { hits: [1, 2], spreadStatus: { kind: 'poison', targets: 1, chance: 20 } }, status: { kind: 'poison', chance: 45, duration: 3 }, range: 9, projectile: { speed: 91, width: 0.2, pierce: 2 }, desc: 'One or two venom-tipped shots, and the venom may pass to a neighbour.' },
    { name: 'Toxin Stack', learnLevel: 180, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 2.6, accuracy: 92, power: 27, mana: 10, variance: 0.15, status: { kind: 'poison', chance: 70, duration: 4 }, range: 8.9, projectile: { speed: 95, width: 0.22, pierce: 0 }, desc: 'A cheap second dose. Poison is the point; the dart barely matters.' },
    ...
    { name: 'Fester', learnLevel: 220, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 2.6, accuracy: 92, power: 28, mana: 8, variance: 0.2, effects: { bonusVsStatus: { kind: 'poison', mult: 1.5, consume: true } }, range: 9.2, projectile: { speed: 87, width: 0.24, pierce: 0 }, desc: 'Digs at a poisoned wound until the venom does the rest.' },
    { name: 'Twin Fangs', learnLevel: 280, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 2.6, accuracy: 90, power: 17, mana: 14, variance: 0.25, effects: { hits: [2, 2] }, status: { kind: 'bleed', chance: 35, duration: 3 }, range: 9, projectile: { speed: 86, width: 0.2, pierce: 0 }, desc: 'Two quick shots that open a wound — bleed here, poison everywhere else in the line.' },
    { name: 'Paralytic Dart', learnLevel: 400, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 5.2, accuracy: 90, power: 53, mana: 22, variance: 0.15, status: { kind: 'stun', chance: 35, duration: 1 }, range: 8.3, projectile: { speed: 95, width: 0.26, pierce: 0 }, desc: "DEX's one hard control: a neurotoxin that drops them where they stand." },
    { name: 'Virulence', learnLevel: 560, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 6.5, accuracy: 90, power: 52, mana: 20, variance: 0.2, effects: { bonusVsStatus: { kind: 'poison', mult: 2.6, consume: true } }, range: 8.4, projectile: { speed: 87, width: 0.28, pierce: 0 }, desc: 'Feeble on clean blood, ruinous on poisoned — the payoff the whole line builds toward.' },
    { name: 'Plague Shot', learnLevel: 740, type: 'damage', channel: 'ranged', target: 'allEnemies', cooldown: 6.5, accuracy: 85, power: 45, mana: 34, variance: 0.2, effects: { spreadStatus: { kind: 'poison', targets: 2, chance: 40 } }, status: { kind: 'poison', chance: 55, duration: 4 }, range: 7.2, projectile: { speed: 72, width: 0.38, pierce: 0 }, desc: 'Poison that jumps between bodies. It rewards an enemy that stands together.' },

    // ── Volley — volume and variance. The widest damage ranges in the game ────
    { name: 'Sling', learnLevel: 40, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 1.3, accuracy: 95, power: 13, mana: 5, variance: 0.3, effects: { hits: [1, 2] }, range: 11, projectile: { speed: 61, width: 0.46, pierce: 0 }, desc: 'One or two quick shots. Cheap enough to throw all day.' },
    ...
    { name: 'Pin Down', learnLevel: 330, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 3.9, accuracy: 88, power: 40, mana: 18, variance: 0.15, range: 10, projectile: { speed: 78, width: 0.47, pierce: 0 }, desc: 'Suppressing fire that drags them out of position and holds them there.' },
    ...
    { name: "Gambler's Volley", learnLevel: 470, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 3.9, accuracy: 85, power: 10, mana: 20, variance: 0.5, effects: { hits: [1, 6] }, range: 11, projectile: { speed: 40, width: 0.42, pierce: 0 }, desc: 'Everything in the quiver, all at once, aimed roughly. Anywhere from a scratch to a slaughter.' },
    { name: 'Ricochet', learnLevel: 500, type: 'damage', channel: 'ranged', target: 'allEnemies', cooldown: 5.2, accuracy: 88, power: 34, mana: 26, variance: 0.3, effects: { hits: [1, 2] }, range: 8.9, projectile: { speed: 50, width: 0.74, pierce: 1 }, desc: 'One shot, several bodies. It bounces, and it does not much care whose.' },
    { name: 'Rain of Arrows', learnLevel: 650, type: 'damage', channel: 'ranged', target: 'allEnemies', cooldown: 6.5, accuracy: 85, power: 52, mana: 34, variance: 0.25, range: 8.4, projectile: { speed: 54, width: 0.82, pierce: 0 }, desc: 'A bombardment onto a chosen patch of ground — it punishes standing together.' },
    { name: 'Pinning Volley', learnLevel: 680, type: 'damage', channel: 'ranged', target: 'allEnemies', cooldown: 6.5, accuracy: 88, power: 48, mana: 32, variance: 0.2, range: 8.5, projectile: { speed: 59, width: 0.76, pierce: 0 }, desc: 'Nails several of them to the spot at once.' },
    ...
    { name: 'Deadeye', learnLevel: 920, type: 'damage', channel: 'ranged', target: 'enemy', cooldown: 7.8, accuracy: 95, power: 131, mana: 44, variance: 0.05, range: 9.2, projectile: { speed: 90, width: 0.46, pierce: 1 }, desc: 'One shot. It goes exactly where it was sent.' },
  ],
  ...
  INT: [
    ...
    { name: 'Ember', learnLevel: 40, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 2.6, accuracy: 90, power: 14, mana: 6, variance: 0.2, status: { kind: 'burn', chance: 45, duration: 3 }, range: 7.9, projectile: { speed: 42, width: 0.49, pierce: 0 }, desc: 'Minor fire, and it catches. The cheapest way to start a stack.' },
    { name: 'Fracturing Stones', learnLevel: 120, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 2.6, accuracy: 90, power: 24, mana: 14, variance: 0.2, status: { kind: 'vulnerable', chance: 40, duration: 3 }, range: 7.5, projectile: { speed: 42, width: 0.54, pierce: 0 }, desc: 'A stinging barrage that cracks the guard. The second stack type.' },
    { name: 'Cinderburst', learnLevel: 200, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 3.9, accuracy: 88, power: 37, mana: 20, variance: 0.15, effects: { bonusVsStatus: { kind: 'burn', mult: 1.6, consume: true } }, range: 7, projectile: { speed: 46, width: 0.55, pierce: 0 }, desc: 'Solid on its own, and it snuffs a Burn for far more. The first detonator.' },
    { name: 'Sap Will', learnLevel: 280, type: 'debuff', channel: 'magic', target: 'enemy', cooldown: 5.2, accuracy: 95, power: 0, mana: 18, effects: { atkDebuff: 0.22, duration: 3 }, range: 7.5, projectile: { speed: 37, width: 0.28, pierce: 0 }, desc: 'Drains the will to strike: −22% damage for 3 rounds. INT could not do this at all before.' },
    { name: 'Arcane Bomb', learnLevel: 340, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 5.2, accuracy: 88, power: 37, mana: 26, variance: 0.2, effects: { bonusVsStatus: { kind: 'vulnerable', mult: 1.7, consume: true } }, range: 6.6, projectile: { speed: 42, width: 0.59, pierce: 0 }, desc: 'A charge left ticking on them — devastating on anything already cracked open.' },
    { name: 'Curse of Ruin', learnLevel: 480, type: 'debuff', channel: 'magic', target: 'enemy', cooldown: 6.5, accuracy: 95, power: 0, mana: 24, effects: { defDebuff: 14, duration: 3 }, range: 7.5, projectile: { speed: 37, width: 0.28, pierce: 0 }, desc: 'Unpicks whatever is holding them together: −14 mitigation from EVERY source of damage.' },
    { name: 'Detonate', learnLevel: 700, type: 'damage', channel: 'magic', target: 'allEnemies', cooldown: 6.5, accuracy: 85, power: 51, mana: 38, variance: 0.25, effects: { bonusVsStatus: { kind: 'burn', mult: 2.0, consume: true } }, range: 5.6, projectile: { speed: 32, width: 0.99, pierce: 0 }, desc: 'Sets off everything still burning, on everyone at once. The Hexer capstone.' },

    // ── Elementalist — ground you cannot stand on ────────────────────────────
    { name: 'Frost Shard', learnLevel: 90, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 2.6, accuracy: 90, power: 23, mana: 12, variance: 0.2, range: 8.3, projectile: { speed: 31, width: 0.85, pierce: 0 }, desc: 'An icy dart. The frost line opens here.' },
    { name: 'Rime Bind', learnLevel: 160, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 3.9, accuracy: 90, power: 34, mana: 16, variance: 0.15, range: 8, projectile: { speed: 34, width: 0.85, pierce: 0 }, desc: 'Ice climbs the legs and sets. It can still cast; it is going nowhere.' },
    { name: 'Frost Nova', learnLevel: 280, type: 'damage', channel: 'magic', target: 'allEnemies', cooldown: 5.2, accuracy: 85, power: 31, mana: 26, variance: 0.2, range: 6.9, projectile: { speed: 26, width: 1.3, pierce: 0 }, desc: 'A ring of hoarfrost bursts outward — the anti-melee tool casters never had.' },
    ...
    { name: 'Inferno', learnLevel: 430, type: 'damage', channel: 'magic', target: 'allEnemies', cooldown: 5.2, accuracy: 82, power: 44, mana: 32, variance: 0.25, status: { kind: 'burn', chance: 30, duration: 3 }, range: 6.6, projectile: { speed: 25, width: 1.49, pierce: 0 }, desc: 'Fire across the whole position, and much of it keeps burning.' },
    { name: 'Seismic Crush', learnLevel: 560, type: 'damage', channel: 'magic', target: 'allEnemies', cooldown: 6.5, accuracy: 82, power: 53, mana: 38, variance: 0.25, status: { kind: 'stun', chance: 30, duration: 1 }, range: 6.3, projectile: { speed: 25, width: 1.56, pierce: 0 }, desc: 'The ground itself comes up. Damage AND a stun on everything standing on it.' },
    { name: 'World Ender', learnLevel: 920, type: 'damage', channel: 'magic', target: 'allEnemies', cooldown: 9.1, accuracy: 78, power: 87, mana: 56, variance: 0.3, effects: { maxHpDmg: 0.02 }, range: 6, projectile: { speed: 25, width: 1.7, pierce: 0 }, desc: 'The largest thing in the game, and it hurts the biggest of them most.' },

    // ── Arcanist — move things. Yourself, or them ────────────────────────────
    { name: 'Spark', learnLevel: 40, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 1.3, accuracy: 95, power: 16, mana: 5, variance: 0.15, range: 7.4, projectile: { speed: 72, width: 0.5, pierce: 1 }, desc: 'A small air bolt, cheap enough to throw between everything else.' },
    ...
    { name: 'Static Chain', learnLevel: 330, type: 'damage', channel: 'magic', target: 'allEnemies', cooldown: 5.2, accuracy: 85, power: 39, mana: 28, variance: 0.2, status: { kind: 'vulnerable', chance: 25, duration: 2 }, range: 6, projectile: { speed: 54, width: 0.88, pierce: 1 }, desc: 'A bolt that leaps body to body along a line, weakening as it goes.' },
    { name: 'Mana Leech', learnLevel: 335, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 3.9, accuracy: 88, power: 34, mana: 22, variance: 0.15, effects: { manaBurn: 14, lifesteal: 0.25 }, range: 6.9, projectile: { speed: 72, width: 0.55, pierce: 1 }, desc: 'Siphons and steps away in the same motion. WIS steals better; this one escapes.' },
    { name: 'Unmake', learnLevel: 560, type: 'debuff', channel: 'magic', target: 'enemy', cooldown: 6.5, accuracy: 95, power: 0, mana: 26, effects: { consumeWard: 1, accDebuff: 12, duration: 3 }, range: 7, projectile: { speed: 58, width: 0.3, pierce: 1 }, desc: 'Strips the shield off them and leaves their aim shaking.' },
    { name: 'Displace', learnLevel: 640, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 5.2, accuracy: 90, power: 50, mana: 26, variance: 0.15, range: 6.8, projectile: { speed: 72, width: 0.6, pierce: 1 }, desc: 'Teleports the TARGET — rips a diver out of your back line and pins it where it lands.' },
    { name: 'Void Lance', learnLevel: 780, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 6.5, accuracy: 85, power: 89, mana: 38, variance: 0.1, effects: { pierce: 0.5 }, range: 6.2, projectile: { speed: 78, width: 0.61, pierce: 2 }, desc: 'Pure void. Half of everything they are wearing simply does not apply.' },
    { name: 'Arcane Overload', learnLevel: 850, type: 'damage', channel: 'magic', target: 'enemy', cooldown: 7.8, accuracy: 85, power: 168, mana: 44, variance: 0.3, effects: { recoil: 0.15 }, range: 6.2, projectile: { speed: 55, width: 0.8, pierce: 3 }, desc: 'Overchannelled past what the caster can hold. It burns them too.' },
  ],
